...
public class ServiceDefinitionList {
	...
	public ServiceDefinition findByType(String javaTypeName) {
		return serviceDefinitions.stream()
				.filter(serviceDefinition -> serviceDefinition.javaTypeName.equals(javaTypeName))
				.findFirst()
				.orElse(null);
	}
	...
	public ServiceDefinition findByTemplate(ServiceDefinition template) {
		return serviceDefinitions.stream()
				...
				.filter(serviceDefinition -> serviceDefinition.javaTypeName.equals(template.javaTypeName))
				.findFirst()
				.orElse(null);
	}
}
